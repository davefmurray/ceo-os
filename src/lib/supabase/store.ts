...
function dbToAppQuarterlyReview(db: DbQuarterlyReview): QuarterlyReview {
  return {
    ...
    startDate: db.start_date,
    endDate: db.end_date,
    goalProgress: (db.goal_progress as QuarterlyReview['goalProgress']) ?? [],
    lifeMapScores: (db.life_map_scores as LifeMapScores) ?? defaultLifeMapScores(),
    averageEnergy: db.average_energy ?? 5,
...
function dbToAppAnnualReview(db: DbAnnualReview): AnnualReview {
  return {
    ...
    gratitude: db.gratitude,
    quotesToRemember: db.quotes_to_remember,
    createdAt: db.created_at,
    updatedAt: db.updated_at,
  };
}
...
function dbToAppNorthStar(db: DbNorthStar): NorthStar {
  return {
    ...
    company: db.company,
    yearsInChapter: db.years_in_chapter,
    whatDefinesChapter: db.what_defines_chapter,
    areaRankings: (db.area_rankings as AreaRanking[]) ?? defaultAreaRankings(),
    nonNegotiables: db.non_negotiables,
    sayingNoTo: db.saying_no_to,
...
function dbToAppLifeMap(db: DbLifeMap): LifeMapScores & { id: string; snapshotDate: string } {
  return {
    ...
    careerTrend: (db.career_trend as LifeMapScores['careerTrend']) ?? 'stable',
    careerNote: db.career_note,
    relationships: db.relationships_score ?? 5,
    relationshipsTrend: (db.relationships_trend as LifeMapScores['relationshipsTrend']) ?? 'stable',
    relationshipsNote: db.relationships_note,
    health: db.health_score ?? 5,
    healthTrend: (db.health_trend as LifeMapScores['healthTrend']) ?? 'stable',
    healthNote: db.health_note,
    finances: db.finances_score ?? 5,
    financesTrend: (db.finances_trend as LifeMapScores['financesTrend']) ?? 'stable',
    financesNote: db.finances_note,
    meaning: db.meaning_score ?? 5,
    meaningTrend: (db.meaning_trend as LifeMapScores['meaningTrend']) ?? 'stable',
    meaningNote: db.meaning_note,
    fun: db.fun_score ?? 5,
    funTrend: (db.fun_trend as LifeMapScores['funTrend']) ?? 'stable',
    funNote: db.fun_note,
  };
}
...
function dbToAppMemory(db: DbMemory): Memory {
  return {
    ...
    growthEdges: db.growth_edges,
    energizedBy: db.energized_by,
    drainedBy: db.drained_by,
    optimalConditions: db.optimal_conditions,
    overIndexes: db.over_indexes,
    underWeights: db.under_weights,
    blindSpots: db.blind_spots,
    goalArchaeology: (db.goal_archaeology as Memory['goalArchaeology']) ?? [],
    insightsByYear: (db.insights_by_year as Memory['insightsByYear']) ?? [],
    lessonsWork: db.lessons_work,
    lessonsRelationships: db.lessons_relationships,
    lessonsSelf: db.lessons_self,
    lessonsLife: db.lessons_life,
    quotesFromPastSelf: (db.quotes_from_past_self as Memory['quotesFromPastSelf']) ?? [],
    warningsToFutureSelf: db.warnings_to_future_self,
...
function dbToAppInterviewResponse(db: DbInterviewResponse): InterviewResponse {
  return {
    ...
    responses: (db.responses as Record<string, string>) ?? {},
...
function dbToAppYearGoals(db: DbGoal): YearGoals {
  return {
    ...
    career: (db.career as GoalArea) ?? defaultGoalArea(),
    relationships: (db.relationships as GoalArea) ?? defaultGoalArea(),
    health: (db.health as GoalArea) ?? defaultGoalArea(),
    finances: (db.finances as GoalArea) ?? defaultGoalArea(),
    meaning: (db.meaning as GoalArea) ?? defaultGoalArea(),
    fun: (db.fun as GoalArea) ?? defaultGoalArea(),
    criticalThree: db.critical_three,
    antiGoals: db.anti_goals,
    habitsToBuild: (db.habits_to_build as YearGoals['habitsToBuild']) ?? [],
    habitsToBreak: (db.habits_to_break as YearGoals['habitsToBreak']) ?? [],
    updatedAt: db.updated_at,
  };
}
...
function dbToAppVisionGoals(db: DbGoal): VisionGoals {
  return {
    ...
    bigBets: db.big_bets,
    needsToEnd: db.needs_to_end,
    needsToBegin: db.needs_to_begin,
    updatedAt: db.updated_at,
  };
}
